...
function(_qt5_ServiceFramework_process_prl_file prl_file_location Configuration lib_deps link_flags)
    set(_lib_deps)
    set(_link_flags)

    set(_qt5_install_libs "${_qt5ServiceFramework_install_prefix}/lib/")
            ...
            elseif(_framework_flag OR _flag MATCHES "^-l(.*)$")
                ...
                elseif(_lib MATCHES "^pthread$")
                    ...
                    if(current_search_paths)
                        find_library(_Qt5ServiceFramework_${Configuration}_${_lib}_PATH ${_lib} HINTS ${current_search_paths} NO_DEFAULT_PATH)
                    endif()
                    find_library(_Qt5ServiceFramework_${Configuration}_${_lib}_PATH ${_lib} HINTS ${CMAKE_CXX_IMPLICIT_LINK_DIRECTORIES})
                    mark_as_advanced(_Qt5ServiceFramework_${Configuration}_${_lib}_PATH)
                    if(_Qt5ServiceFramework_${Configuration}_${_lib}_PATH)
                        list(APPEND _lib_deps
                            ${_Qt5ServiceFramework_${Configuration}_${_lib}_PATH}
                        )
                    else()
...
macro(_populate_ServiceFramework_target_properties Configuration LIB_LOCATION IMPLIB_LOCATION
    ...
    set(_static_deps
        ${_Qt5ServiceFramework_STATIC_${Configuration}_LIB_DEPENDENCIES}
    )
    ...
    set(_static_link_flags "${_Qt5ServiceFramework_STATIC_${Configuration}_LINK_FLAGS}")
    if(_static_link_flags)
        set(_static_link_flags_genex "$<${_genex_condition}:${_static_link_flags}>")
        if(NOT CMAKE_VERSION VERSION_LESS "3.13")
            set_property(TARGET Qt5::ServiceFramework APPEND PROPERTY INTERFACE_LINK_OPTIONS
...
if (NOT TARGET Qt5::ServiceFramework)
    ...
    if(NOT Qt5_EXCLUDE_STATIC_DEPENDENCIES)
        ...
        _qt5_ServiceFramework_process_prl_file(
            "${_qt5ServiceFramework_install_prefix}/lib/Qt5ServiceFramework.prl" RELEASE
            _Qt5ServiceFramework_STATIC_RELEASE_LIB_DEPENDENCIES
            _Qt5ServiceFramework_STATIC_RELEASE_LINK_FLAGS
        )
    endif()

    add_library(Qt5::ServiceFramework STATIC IMPORTED)
    set_property(TARGET Qt5::ServiceFramework PROPERTY IMPORTED_LINK_INTERFACE_LANGUAGES CXX)
    ...
    _populate_ServiceFramework_target_properties(RELEASE "Qt5ServiceFramework.lib" "" FALSE)

    if (EXISTS "${_qt5ServiceFramework_install_prefix}/lib/Qt5ServiceFrameworkd.lib" )
        _populate_ServiceFramework_target_properties(DEBUG "Qt5ServiceFrameworkd.lib" "" FALSE)
    endif()
