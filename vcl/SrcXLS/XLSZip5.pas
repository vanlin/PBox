...
type
  TXLSZipOpenMode = (xzomClosed, xzomRead, xzomWrite);

type
  TXLSZipArchive = class;
...
begin
  ...
  if (FCurrZip.FCentralDirectoryFile.CompressionMethod = 8) or (FCurrZip.FCentralDirectoryFile.CompressionMethod = 9) then
  begin
    case FCurrZip.FCentralDirectoryFile.GeneralPurposeBitFlag AND 6 of
      0:
        FCurrZip.FCompressionType := ctNormal;
      2:
        FCurrZip.FCompressionType := ctMaximum;
      4:
        FCurrZip.FCompressionType := ctFast;
      6:
        FCurrZip.FCompressionType := ctSuperFast
    end;
  end;
...
begin
  ...
  with FCurrZip.FCentralDirectoryFile do
  begin
    CentralFileHeaderSignature  := $02014B50;
    VersionMadeBy               := 20;
    VersionNeededToExtract      := 20;
...
begin
  ...
  FStream.Read(Byte_, 4);
  if (Byte_[0] = $50) and (Byte_[1] = $4B) and (Byte_[2] = $03) and (Byte_[3] = $04) then
  begin
    FStream.Position := Offset;
...
begin
  ...
  try
    ...
    for i            := 0 to FEndOfCentralDir.TotalNumberOfEntriesOnThisDisk - 1 do
    begin
...
begin
  if FindCentralDirectory then
  begin
    if ParseCentralHeaders then
    begin
      LoadLocalHeaders;
    end
    else
...
begin
  Result := SizeOf(TCentralDirectoryFile) - 3 * SizeOf(AnsiString) + FCentralDirectoryFile.FilenameLength + FCentralDirectoryFile.ExtraFieldLength + FCentralDirectoryFile.FileCommentLength;
end;
...
begin
  ...
  case FCentralDirectoryFile.CompressionMethod of
    0:
      FDStream := FParent.FStream;
    8:
      FDStream := TDecompressionStream.CreateZip(FParent.FStream, FLocalFile.CompressedSize);
  end;
  // FDStream.Seek(0,0);
end;
...
{$IFDEF DELPHI_6}
  ...
  if (Offset = 0) and (Origin = soEnd) then
  begin
    Result     := FLocalFile.UncompressedSize;
    FSeekCount := 1;
  end
