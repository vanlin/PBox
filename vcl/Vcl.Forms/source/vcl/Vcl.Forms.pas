...
{$ENDIF}
  ...
  Vcl.GraphUtil, Winapi.UxTheme, Winapi.ShlObj;
...
begin
  inherited;
  if FCustomTitleBar.Enabled and FCustomTitleBar.ShowIcon then
    ...
    case Message.Result of
      HTCLIENT:
        begin
          ...
          else if LPoint.Y < FCustomTitleBar.FrameRect.Right then
...
begin
  Result := not ((Message.Section <> nil) and ((StrComp(Message.Section, 'Policy') = 0) or
   (StrComp(Message.Section, 'Windows') = 0) or (StrComp(Message.Section, 'ImmersiveColorSet') = 0) or
...
begin
  ...
  begin
    ...
    Inc(HintInfo.HintPos.Y, MulDiv(GetCursorHeightMargin, Monitor.PixelsPerInch, Screen.PixelsPerInch));
...
begin
  Result := TRect.Create(0, 0, Height, FClient.ClientRect.Width);
end;
    ...
    try
      ...
      begin
        ...
        begin
          ...
          LStyle.DrawElement(B.Canvas.Handle, Details, R, nil, LPPI);
        end;

        if (VertSliderRect.Height > 0) and (VertSliderRect.Width > 0) then
        begin
          Details := LStyle.GetElementDetails(VertSliderState);
          LStyle.DrawElement(B.Canvas.Handle, Details, VertSliderRect, nil, LPPI);
        end;
        ...
        LStyle.DrawElement(B.Canvas.Handle, Details, VertUpButtonRect, nil, LPPI);

        if VertSliderRect.Height <> 0 then
          Details := LStyle.GetElementDetails(FVertDownState)
        else
          Details := LStyle.GetElementDetails(tsArrowBtnDownDisabled);
        LStyle.DrawElement(B.Canvas.Handle, Details, VertDownButtonRect, nil, LPPI);
      end;
      MoveWindowOrg(B.Canvas.Handle, VertScrollRect.Left, VertScrollRect.Top);
    ...
    try
      ...
      begin
        ...
        begin
          ...
          LStyle.DrawElement(B.Canvas.Handle, Details, HorzSliderRect, nil, LPPI);
        end;
        ...
        LStyle.DrawElement(B.Canvas.Handle, Details, HorzUpButtonRect, nil, LPPI);

        if HorzSliderRect.Height > 0 then
          Details := LStyle.GetElementDetails(FHorzDownState)
        else
          Details := LStyle.GetElementDetails(tsArrowBtnRightDisabled);
        LStyle.DrawElement(B.Canvas.Handle, Details, HorzDownButtonRect, nil, LPPI);
      end;
      MoveWindowOrg(B.Canvas.Handle, HorzScrollRect.Left, HorzScrollRect.Top);
...
begin
  ...
  begin
    ...
    if CheckPerMonitorV2SupportForWindow(Form.Handle) and
      (Screen.PixelsPerInch <> LPPI) then
    begin
