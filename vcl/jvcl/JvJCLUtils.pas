...
{ returns date relative to current date: 'дв?дня наза? }
...
function Win2Koi(const S: AnsiString): AnsiString;
const
  W: AnsiString = 'абвгдеёжзийклмнопр?уфхчцшщьыъэЭя+--+-+ЁЖ?+--?+ПРСТУФiЧЦ?_?ЭЮ?';
  K: AnsiString = '--ЧЗ-+ЈЦ++--?+ПРТУФi??ЭЭ?о_+СбвчздеЭцъйклмнопр=уфхжиЭгыэш?ьа?';
var
  I, J: Integer;
begin
  Result := S;
...
const
  ...
  Month: array [1..3] of string =
    ('ме??', 'ме?ца', 'ме?це?'); // Month, Months, Months
  Year: array [1..3] of string =
    ('го?', 'года', 'ле='); // Year, Years, Years
  Week: array [1..4] of string =
    ('неделЭ', '2 недели', '3 недели', 'ме??'); // Week, 2 Weeks, 3 Weeks, Month
var
  Y, M, D: Integer;
begin
  if Date = Dat then
  ...
  if Dat = Date - 1 then
    Result := 'вчер?' // Yesterday
  else
  if Dat = Date - 2 then
    Result := 'позавчер?' // Day before yesterday
  else
  if Dat > Date then
    Result := '?будуще?' // In the future
  else
  begin
    D := Trunc(Date - Dat);
    Y := Round(D / 365);
    M := Round(D / 30);
    if Y > 0 then
      Result := IntToStr(Y) + ' ' + Year[D2D[StrToInt(IntToStr(Y)[Length(IntToStr(Y))])]] + ' наза?' // ago
    else
    if M > 0 then
      Result := IntToStr(M) + ' ' + Month[D2D[StrToInt(IntToStr(M)[Length(IntToStr(M))])]] + ' наза?' // ago
    else
    if D > 6 then
      Result := Week[D div 7] + ' наза?' // ago
    else
    if D > 0 then
      Result := IntToStr(D) + ' ' + Day[D2D[StrToInt(IntToStr(D)[Length(IntToStr(D))])]] + ' наза?' // ago
  end;
end;
