...
function(_qt5_Feedback_process_prl_file prl_file_location Configuration lib_deps link_flags)
    set(_lib_deps)
    set(_link_flags)

    set(_qt5_install_libs "${_qt5Feedback_install_prefix}/lib/")
            ...
            elseif(_framework_flag OR _flag MATCHES "^-l(.*)$")
                ...
                elseif(_lib MATCHES "^pthread$")
                    ...
                    if(current_search_paths)
                        find_library(_Qt5Feedback_${Configuration}_${_lib}_PATH ${_lib} HINTS ${current_search_paths} NO_DEFAULT_PATH)
                    endif()
                    find_library(_Qt5Feedback_${Configuration}_${_lib}_PATH ${_lib} HINTS ${CMAKE_CXX_IMPLICIT_LINK_DIRECTORIES})
                    mark_as_advanced(_Qt5Feedback_${Configuration}_${_lib}_PATH)
                    if(_Qt5Feedback_${Configuration}_${_lib}_PATH)
                        list(APPEND _lib_deps
                            ${_Qt5Feedback_${Configuration}_${_lib}_PATH}
                        )
                    else()
...
macro(_populate_Feedback_target_properties Configuration LIB_LOCATION IMPLIB_LOCATION
    ...
    set(_static_deps
        ${_Qt5Feedback_STATIC_${Configuration}_LIB_DEPENDENCIES}
    )
    ...
    set(_static_link_flags "${_Qt5Feedback_STATIC_${Configuration}_LINK_FLAGS}")
    if(_static_link_flags)
        set(_static_link_flags_genex "$<${_genex_condition}:${_static_link_flags}>")
        if(NOT CMAKE_VERSION VERSION_LESS "3.13")
            set_property(TARGET Qt5::Feedback APPEND PROPERTY INTERFACE_LINK_OPTIONS
...
if (NOT TARGET Qt5::Feedback)
    ...
    if(NOT Qt5_EXCLUDE_STATIC_DEPENDENCIES)
        ...
        _qt5_Feedback_process_prl_file(
            "${_qt5Feedback_install_prefix}/lib/Qt5Feedback.prl" RELEASE
            _Qt5Feedback_STATIC_RELEASE_LIB_DEPENDENCIES
            _Qt5Feedback_STATIC_RELEASE_LINK_FLAGS
        )
    endif()

    add_library(Qt5::Feedback STATIC IMPORTED)
    set_property(TARGET Qt5::Feedback PROPERTY IMPORTED_LINK_INTERFACE_LANGUAGES CXX)
    ...
    _populate_Feedback_target_properties(RELEASE "Qt5Feedback.lib" "" FALSE)

    if (EXISTS "${_qt5Feedback_install_prefix}/lib/Qt5Feedbackd.lib" )
        _populate_Feedback_target_properties(DEBUG "Qt5Feedbackd.lib" "" FALSE)
    endif()
